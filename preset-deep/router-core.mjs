...
/**
 * WEAK_FLASH_DEEP —— 切断回灌配套的长思考引导（v2）。
 *
 * 为什么需要：opencode-go 网关切断思维链回灌后（见 README「回灌切断」），
 * flash 模型失去"看到自己历史长链"的激励，思考会变浅（实测：思考总量
 * 245K→110K、建模质量 30→24）。本 persona 用**显式指令**替代回灌激励，
 * 主动要求多阶段推演、自我质疑、交叉验证，把思考深度拉回（甚至超过）
 * 未切断水平，同时保持 let me 语域干净（不再被历史链带偏）。
 *
 * 关键锚：
 *   1. 分阶段推演（多 pass）：先框架 → 再细节 → 再验证；
 *   2. 自我质疑（challenge/red-team）：每个结论都要反向检验；
 *   3. 交叉验证（cross-check）：用第二种方法复核关键数值；
 *   4. 收敛：最终产出明确结论（不无限发散）。
 */
export const WEAK_FLASH_DEEP =
  'You are a helpful assistant.\n'
  + 'Before acting, decide the task type (build or fix) and adopt the matching '
  + 'style: build → hands-on production; fix → inspect-and-plan.\n'
  ...
  + 'Do not run environment checks (echo, whoami, uname, node --version, '
  + 'date) or exhaustive grep/glob scans. Do not narrate your reasoning as '
  + '"Let me…" chatter; reason in structured passes and end each pass with a '
  + 'decision or an information need.'
 ...
 * @param mode - the routed mode ('weak' | 0..1).
 * @param modelId - the routed model id.
 * @param options - optional { deepThinking } — when true, flash uses
 *   WEAK_FLASH_DEEP (explicit multi-pass long-thinking guidance, for the
 *   replay-cutoff build); when false/undefined, the classic WEAK_FLASH.
 */
export function personaFor(mode, modelId, options) {
  switch (bandOf(mode)) {
    case 'spec': return SPEC_PERSONA
    case 'transition': return MIXED_PERSONA
    case 'weak': return isFlashModel(modelId)
      ? (options?.deepThinking ? WEAK_FLASH_DEEP : WEAK_FLASH)
      : WEAK_PRO
