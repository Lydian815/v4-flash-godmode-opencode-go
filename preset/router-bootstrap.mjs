...
import {
  applyPersona, bandOf, bandFor, coreFor, parseMode, personaFor, sessionMode, testinessFor, clamp01,
  isFlashModel,
...
export function apply(ctx, config) {
  ...
  // dsh 适配说明：作者原版用 `ctx.on('session/event')` + `target.inbox.append`
  // 注入近距离引导，但 dsh rc.6 里 session/event 是 session-scoped、agent 对象
  // 无 inbox 属性，且 assemble 时 session.events 里还没有 user/message（消息
  // 在 inbox claim 之后、事件落盘之前）。agent/pre-step 虽能拿到 messages，
  // 但拿不到 agent/model 判断。故改为把深度引导直接合并进 WEAK_FLASH persona
  // （router-core.mjs），静态注入，同样覆盖作者的"分类 + 回顾 + 反跑题 + 深度
  // 思考"四个锚。对固定复杂任务（柴油机）而言静态引导已足够。
