...
export function apply(ctx, config) {
  ...
  // deepThinking: true → Flash 用 WEAK_FLASH_DEEP（显式多阶段长思考引导），
  // 配套"切断回灌"使用（回灌切断后 flash 思考会变浅，用 persona 指令补回）。
  // 默认 false → 经典 WEAK_FLASH（兼容原版 w7 行为）。
  const deepThinking = config?.deepThinking === true
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    const persona = personaFor(mode, modelId, { deepThinking })
